...
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod, esp_bt_uuid_t *uuid, uint8_t *name, uint8_t name_len, int rssi) {
    ...
    if (r) {
        ...
        if (r->name == NULL && name && name_len) {
            ...
            if (name_s == NULL) {
                ESP_LOGE(TAG_APP, "Malloc result name failed!");
                return;
            }
    ...
    if (r == NULL) {
        ESP_LOGE(TAG_APP, "Malloc bt_hidh_scan_result_t failed!");
        return;
    }
...
static void add_ble_scan_result(esp_bd_addr_t bda, esp_ble_addr_type_t addr_type, uint16_t appearance, uint8_t *name, uint8_t name_len, int rssi) {
    if (find_scan_result(bda, ble_scan_results)) {
        ESP_LOGW(TAG_APP, "Result already exists!");
        return;
    }
    esp_hid_scan_result_t *r = (esp_hid_scan_result_t *)malloc(sizeof(esp_hid_scan_result_t));
    if (r == NULL) {
        ESP_LOGE(TAG_APP, "Malloc ble_hidh_scan_result_t failed!");
        return;
    }
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param) {
    ...
    case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: {
        ESP_LOGV(TAG_APP, "BT GAP DISC_STATE %s", (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STARTED) ? "START" : "STOP");
    ...
    case ESP_BT_GAP_KEY_NOTIF_EVT:
        ESP_LOGI(TAG_APP, "BT GAP KEY_NOTIF passkey:%"PRIu32, param->key_notif.passkey);
        break;
    case ESP_BT_GAP_CFM_REQ_EVT: {
        ESP_LOGI(TAG_APP, "BT GAP CFM_REQ_EVT Please compare the numeric value: %"PRIu32, param->cfm_req.num_val);
        esp_bt_gap_ssp_confirm_reply(param->cfm_req.bda, true);
        break;
    }
    case ESP_BT_GAP_KEY_REQ_EVT:
        ESP_LOGI(TAG_APP, "BT GAP KEY_REQ_EVT Please enter passkey!");
        break;
#endif
    case ESP_BT_GAP_MODE_CHG_EVT:
        ESP_LOGI(TAG_APP, "BT GAP MODE_CHG_EVT mode:%d", param->mode_chg.mode);
        break;
    case ESP_BT_GAP_PIN_REQ_EVT: {
        ESP_LOGI(TAG_APP, "BT GAP PIN_REQ_EVT min_16_digit:%d", param->pin_req.min_16_digit);
        if (param->pin_req.min_16_digit) {
            ESP_LOGI(TAG_APP, "Input pin code: 0000 0000 0000 0000");
            esp_bt_pin_code_t pin_code = {0};
            esp_bt_gap_pin_reply(param->pin_req.bda, true, 16, pin_code);
        } else {
            ESP_LOGI(TAG_APP, "Input pin code: 1234");
    ...
    default:
        ESP_LOGE(TAG_APP, "BT GAP EVENT: %s, [esp_bt_gap_cb_event_t %d]", bt_gap_evt_str(event), event);
        // ESP_LOGW(TAG_APP, "BT GAP EVENT: %s", bt_gap_evt_str(event));
        break;
    }
} /**/
...
static esp_err_t start_bt_scan(uint32_t seconds) {
    ...
    if ((ret = esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, (int)(seconds / 1.28), 0)) != ESP_OK) {
        ESP_LOGE(TAG_APP, "esp_bt_gap_start_discovery failed: %d", ret);
        return ret;
    }
    return ret;
} /**/
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ESP_LOGV(TAG_APP, "BLE GAP EVENT SCAN_PARAM_SET_COMPLETE");
        SEND_BLE_CB();
        break;
    }
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            ESP_LOGV(TAG_APP, "BLE GAP EVENT SCAN DONE: %d", scan_result->scan_rst.num_resps);
            SEND_BLE_CB();
            break;
        default:
            break;
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT: {
        ESP_LOGV(TAG_APP, "BLE GAP EVENT SCAN CANCELED");
        break;
    }
    /**
     * ADVERTISEMENT
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
        ESP_LOGV(TAG_APP, "BLE GAP ADV_DATA_SET_COMPLETE");
        break;

    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ESP_LOGV(TAG_APP, "BLE GAP ADV_START_COMPLETE");
        break;
    /**
     * AUTHENTICATION
     */
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ESP_LOGE(TAG_APP, "BLE GAP AUTH ERROR: 0x%x", param->ble_security.auth_cmpl.fail_reason);
        } else {
            ESP_LOGI(TAG_APP, "BLE GAP AUTH SUCCESS");
        }
        break;
    case ESP_GAP_BLE_KEY_EVT: //shows the ble key info share with peer device to the user.
        ESP_LOGI(TAG_APP, "BLE GAP KEY type = %s", esp_ble_key_type_str(param->ble_security.ble_key.key_type));
        break;
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // ESP_IO_CAP_OUT
        // The app will receive this evt when the IO has Output capability and the peer device IO has Input capability.
        // Show the passkey number to the user to input it in the peer device.
        ESP_LOGI(TAG_APP, "BLE GAP PASSKEY_NOTIF passkey:%"PRIu32, param->ble_security.key_notif.passkey);
        break;
    case ESP_GAP_BLE_NC_REQ_EVT: // ESP_IO_CAP_IO
        // The app will receive this event when the IO has DisplayYesNO capability and the peer device IO also has DisplayYesNo capability.
        // show the passkey number to the user to confirm it with the number displayed by peer device.
        ESP_LOGI(TAG_APP, "BLE GAP NC_REQ passkey:%"PRIu32, param->ble_security.key_notif.passkey);
    ...
    case ESP_GAP_BLE_PASSKEY_REQ_EVT: // ESP_IO_CAP_IN
        ...
        ESP_LOGI(TAG_APP, "BLE GAP PASSKEY_REQ");
        //esp_ble_passkey_reply(param->ble_security.ble_req.bd_addr, true, 1234);
        break;
    case ESP_GAP_BLE_SEC_REQ_EVT:
        ESP_LOGI(TAG_APP, "BLE GAP SEC_REQ");
        // Send the positive(true) security response to the peer device to accept the security request.
        // If not accept the security request, should send the security response with negative(false) accept value.
        esp_ble_gap_security_rsp(param->ble_security.ble_req.bd_addr, true);
        break;
    default:
        ESP_LOGV(TAG_APP, "BLE GAP EVENT %s", ble_gap_evt_str(event));
        break;
    }
} /**/
#endif
...
static esp_err_t init_ble_gap(void) {
    ...
    if ((ret = esp_ble_gap_register_callback(ble_gap_event_handler)) != ESP_OK) {
        ESP_LOGE(TAG_APP, "esp_ble_gap_register_callback failed: %d", ret);
        return ret;
    }
    return ret;
} /**/
...
static esp_err_t start_ble_scan(uint32_t seconds) {
    ...
    if ((ret = esp_ble_gap_set_scan_params(&hid_scan_params)) != ESP_OK) {
        ESP_LOGE(TAG_APP, "esp_ble_gap_set_scan_params failed: %d", ret);
        return ret;
    }
    WAIT_BLE_CB();
    if ((ret = esp_ble_gap_start_scanning(seconds)) != ESP_OK) {
        ESP_LOGE(TAG_APP, "esp_ble_gap_start_scanning failed: %d", ret);
        return ret;
    }
    return ret;
} /**/
...
static esp_err_t init_low_level(uint8_t mode) {
    ...
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bt_controller_mem_release failed: %d", ret);
        return ret;
    }
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bt_controller_init failed: %d", ret);
        return ret;
    }
    ret = esp_bt_controller_enable(mode);
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bt_controller_enable failed: %d", ret);
        return ret;
    }
    ret = esp_nimble_init();
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_nimble_init failed: %d", ret);
        return ret;
    }
    return ret;
} /**/
...
nimble_hid_gap_event(struct ble_gap_event *event, void *arg) {
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        ESP_LOGI(TAG_APP, "connection %s; status=%d",
                event->connect.status == 0 ? "established" : "failed",
                event->connect.status);
        return 0;
        break;
    case BLE_GAP_EVENT_DISCONNECT:
        ESP_LOGI(TAG_APP, "disconnect; reason=%d", event->disconnect.reason);
        return 0;
    case BLE_GAP_EVENT_CONN_UPDATE:
        /* The central has updated the connection parameters. */
        ESP_LOGI(TAG_APP, "connection updated; status=%d",
                event->conn_update.status);
        return 0;
    case BLE_GAP_EVENT_ADV_COMPLETE:
        ESP_LOGI(TAG_APP, "advertise complete; reason=%d",
                event->adv_complete.reason);
        return 0;
    case BLE_GAP_EVENT_SUBSCRIBE:
        ESP_LOGI(TAG_APP, "subscribe event; conn_handle=%d attr_handle=%d "
    ...
    case BLE_GAP_EVENT_MTU:
        ESP_LOGI(TAG_APP, "mtu update event; conn_handle=%d cid=%d mtu=%d",
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        ESP_LOGI(TAG_APP, "PASSKEY_ACTION_EVENT started");
        struct ble_sm_io pkey = {0};
        int key = 0;
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            pkey.action = event->passkey.params.action;
            pkey.passkey = 123456; // This is the passkey to be entered on peer
            ESP_LOGI(TAG_APP, "Enter passkey %" PRIu32 "on the peer side", pkey.passkey);
            rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
            ESP_LOGI(TAG_APP, "ble_sm_inject_io result: %d", rc);
        } else if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
            ESP_LOGI(TAG_APP, "Accepting passkey..");
            pkey.action = event->passkey.params.action;
            ...
            ESP_LOGI(TAG_APP, "Input not supported passing -> 123456");
            pkey.action = event->passkey.params.action;
            pkey.passkey = 123456;
            rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
            ESP_LOGI(TAG_APP, "ble_sm_inject_io result: %d", rc);
...
#else
static esp_err_t init_low_level(uint8_t mode) {
    ESP_LOGI(TAG_APP, "init_low_level(%d)    Begin", mode);
    esp_err_t ret;
    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
#if CONFIG_IDF_TARGET_ESP32
    bt_cfg.mode = mode;
    ...
    ESP_LOGI(TAG_APP, "esp_bt_controller_init()");
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bt_controller_init failed: %d", ret);
        return ret;
    }
    ESP_LOGI(TAG_APP, "esp_bt_controller_enable()");
    ret = esp_bt_controller_enable(mode);
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bt_controller_enable failed: %d", ret);
        return ret;
    ...
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bluedroid_init failed: %d", ret);
        return ret;
    }

    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(TAG_APP, "esp_bluedroid_enable failed: %d", ret);
        return ret;
    }
#if CONFIG_BT_HID_HOST_ENABLED
    if (mode & ESP_BT_MODE_CLASSIC_BT) {
...
esp_err_t esp_hid_gap_init(uint8_t mode) {
    ESP_LOGI(TAG_APP, "esp_hid_gap_init(%d)  Begin", mode);
    esp_err_t ret;
    if (!mode || mode > ESP_BT_MODE_BTDM) {
        ESP_LOGE(TAG_APP, "Invalid mode given!");
        return ESP_FAIL;
    }
    if (bt_hidh_cb_semaphore != NULL) {
        ESP_LOGE(TAG_APP, "Already initialised");
        return ESP_FAIL;
    }
    bt_hidh_cb_semaphore = xSemaphoreCreateBinary();
    if (bt_hidh_cb_semaphore == NULL) {
        ESP_LOGE(TAG_APP, "xSemaphoreCreateMutex failed!");
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(TAG_APP, "esp_hid_gap_init()  End");
    return ESP_OK;
} /**/
...
esp_err_t esp_hid_scan(uint32_t seconds, size_t *num_results, esp_hid_scan_result_t **results) {
    if (num_bt_scan_results || bt_scan_results || num_ble_scan_results || ble_scan_results) {
        ESP_LOGE(TAG_APP, "There are old scan results. Free them first!");
        return ESP_FAIL;
    }
#if CONFIG_BT_BLE_ENABLED
